...
class Tailor(Action):
    def __init__(self, entity: Entity, crewman: Crewman):
        """
        this action directs the crewman to attempt repairs on the sails
    ...
    def perform(self) -> bool:
        ...
        if self.entity.sails.hp >= self.entity.sails.max_hp:
            raise Impossible(f"{self.entity.sails.name.capitalize()} is not in need of repairs")
        ...
        self.entity.sails.hp += 1
        self.engine.message_log.add_message(f"{self.crewman.name} repaired {self.entity.sails.name} for 1 point")
